...
def select(input1):
    if input1 == 'All Sites':
        ...
        new_df = spacex_df[spacex_df["Launch Site"] == input1]["class"].value_counts().to_frame()
        new_df["name"] = ["Failure", "Success"]
        fig = px.pie(new_df, values='class', names='name', title='Total Success Launches for ' + input1)
    return fig
# TASK 4:
